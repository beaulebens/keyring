...
class Keyring_Service_GoogleAnalytics extends Keyring_Service_OAuth2 {
	...
	function __construct() {
		...
		if ( ! KEYRING__HEADLESS_MODE ) {
			add_action( 'keyring_google-analytics_manage_ui', array( $this, 'basic_ui' ) );
		...
		add_filter( 'keyring_google-analytics_request_token_params', array( $this, 'request_token_params' ) );
		add_action( 'pre_keyring_google-analytics_verify', array( $this, 'redirect_incoming_verify' ) );
	}
	...
	function basic_ui_intro() {
		...
		echo '<p>' . __( "Now you need to enable the Analytics API and setup your OAuth credentials.", 'keyring' );
		echo '<ol>';
		...
		echo '<li>' . __( "Find and click <strong>Analytics API</strong>.", 'keyring' ) . '</li>';
